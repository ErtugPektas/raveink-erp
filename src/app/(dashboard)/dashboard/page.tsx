...
import { MOCK_APPOINTMENTS, ARTISTS, STATUS_LABELS, type AppointmentStatus } from "@/lib/data";
import { CalendarDays, TrendingUp, Users, CheckCircle, Clock, XCircle, AlertCircle } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const todayAppts = useMemo(
    () => MOCK_APPOINTMENTS.filter(a => a.date === today),
    [today]
  );

  const todayRevenue = useMemo(
    () => todayAppts.filter(a => a.status === "done").reduce((s, a) => s + a.price, 0),
    [todayAppts]
  );
  ...
  const recent = MOCK_APPOINTMENTS.slice(0, 5);
      ...
      <div className="erp-content">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "1rem", alignItems: "start" }}>
          ...
          <div className="card">
            ...
            {todayAppts.length === 0 ? (
              <div style={{ fontFamily: "Montserrat, sans-serif", fontSize: 12, color: "rgba(255,255,255,0.3)", textAlign: "center", padding: "1.5rem 0" }}>
                Bugün randevu yok
              </div>
            ) : (
                ...
                {todayAppts.sort((a, b) => a.time.localeCompare(b.time)).map(a => (
                  <div key={a.id} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontFamily: "Montserrat, sans-serif", fontSize: 11, color: "rgba(255,255,255,0.35)" }}>
                        {a.time} · {a.artistName}
                      </div>
                    </div>
